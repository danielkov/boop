#!/bin/sh
# boop installation script for Unix-like systems (macOS, Linux)
# Usage: curl -sSfL https://raw.githubusercontent.com/danielkov/boop/main/scripts/install.sh | sh

set -e

REPO="danielkov/boop"
BINARY_NAME="boop"
INSTALL_DIR="${BOOP_INSTALL_DIR:-$HOME/.boop/bin}"
...
get_latest_version() {
    ...
    if [ -z "$releases" ]; then
        error "Could not fetch releases. Check https://github.com/${REPO}/releases"
    fi
...
main() {
    ...
    success "boop ${VERSION} installed successfully!"
    ...
    echo ""
    info "Get started with: boop --help"
}
